...
//#define MAX_CELL 14
//#define ZONE_COUNT 42
#define MAX_CELL 7
#define ZONE_COUNT 21
...
struct zns_ftl {
	...
	// [CY] page type per size | zone_count : 42
	uint32_t LSB_reset[MAX_CELL];
	uint32_t LSB_f;
	uint32_t LSB_r;
	uint32_t LSB_u;
	uint32_t CSB_reset[MAX_CELL];
	uint32_t CSB_f;
	uint32_t CSB_r;
	uint32_t CSB_u;
	uint32_t MSB_reset[MAX_CELL];
	uint32_t MSB_f;
	uint32_t MSB_r;
	uint32_t MSB_u;
	uint32_t ZT_list[ZONE_COUNT];
	uint32_t reset_req_list[ZONE_COUNT];
	uint32_t waiting_reset;
	uint32_t RU_list[ZONE_COUNT];

	uint64_t write_traffic[6];
	uint32_t stream;
