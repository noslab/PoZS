...
#define LSB_TIME 10000000000ULL //10s
...
uint64_t ssd_advance_nand(struct ssd *ssd, struct nand_cmd *ncmd)
{
	...
	uint64_t update_time = ptime;
	uint64_t cur_time = cpu_clock(ssd->cpu_nr_dispatcher);

	if (cur_time - ptime > LSB_TIME) {
		printk("LSB_count : %lld\n", CY_LSB_write_count);
		ptime=cur_time;
	}
...
//		nand_etime = nand_stime + spp->pg_wr_lat;
		...
		// 500us / LSB / 500000
		case 0:
			nand_etime = nand_stime + 500000;
			CY_LSB_write_count++;
			break;
		// 5500us / MSB / 5500000
		case 2:
			nand_etime = nand_stime + 5500000;
			CY_MSB_write_count++;
			break;
		// 2000us / CSB / 2000000
		case 1:
			nand_etime = nand_stime + 2000000;
			CY_CSB_write_count++;
			break;
		}
//
