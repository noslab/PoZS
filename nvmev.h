...
#ifndef CY_H
#define CY_H
extern uint64_t CY_write;
extern uint64_t CY_read;
extern uint32_t total_reset;
extern uint32_t user_reset;

extern uint64_t CY_LSB_count;
extern uint64_t CY_CSB_count;
extern uint64_t CY_MSB_count;

extern uint64_t CY_LSB_write_count;
extern uint64_t CY_CSB_write_count;
extern uint64_t CY_MSB_write_count;

extern uint64_t Empty_LSB_reset;
extern uint64_t Empty_CSB_reset;
extern uint64_t Empty_MSB_reset;

extern uint32_t active_lsb;
extern uint32_t active_csb;
extern uint32_t active_msb;

extern uint64_t cy_IO;
extern uint64_t cy_remap;
#endif

#define CY_DEBUG 0

#if CY_DEBUG
#define CY_DEBUG_PRINT(fmt, ...) printk(KERN_INFO fmt, ##__VA_ARGS__)
#else
#define CY_DEBUG_PRINT(fmt, ...)
#endif
