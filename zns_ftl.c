...
bool zns_proc_nvme_io_cmd(struct nvmev_ns *ns, struct nvmev_request *req, struct nvmev_result *ret)
{
	...
	struct nvme_rw_command *e_cmd = &(req->cmd->rw);
	uint64_t nr_lba = e_cmd->length + 1;
	uint32_t reset_unit = 0;

	NVMEV_ASSERT(ns->csi == NVME_CSI_ZNS);
	/*still not support multi partitions ...*/
	NVMEV_ASSERT(ns->nr_parts == 1);

	// [CY] 리셋 요청이 70% 이상 있을 경우 or LSB, CSB, MSB 한 묶음 있을 때
	zns_ftl->reset_req = 0;

	if (zns_ftl->LSB_r > 0 && zns_ftl->CSB_r > 0 && zns_ftl->MSB_r > 0)
		reset_unit = 1;

	if (zns_ftl->waiting_reset >= (ZONE_COUNT/3*2) || reset_unit == 1) {
		CY_DEBUG_PRINT("io_cmd_reset_req\n");
		reset_unit = 0;
		zns_ftl->reset_req = 1;
		zns_zmgmt_send(ns, req, ret);
		CY_DEBUG_PRINT("requset reset\n");
	}
